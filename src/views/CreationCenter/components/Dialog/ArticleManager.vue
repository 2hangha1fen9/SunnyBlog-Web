            ...
            <el-table-column fixed="right" width="180" label="操作">
                <template #default="scope">
                    <el-button-group>
                        ...
                        <el-popconfirm title="您确定要将改文章移入回收站" @confirm="handleSingleDelete(scope.row.id, scope.$index)">
                            <template #reference>
...
import { reactive, watch, ref, computed, onMounted, getCurrentInstance } from "vue"
...
//api
import { listMyArticle, deleteArticle } from "@/api/article/article"
//接口
...
const getArticleList = throttle(function () {
    ...
    listMyArticle(state.pageIndex, state.pageSize, searchCondidtion.value).then((data: Response<PageBean<Array<Article>>>) => {
        if (data.status !== 200) {
            ElMessage.warning("数据拉取失败")
        }
        //将结果保存
        state.page = data.result.page.filter((item: Article) => item.status !== 3)
...
function handleSingleDelete(id: number, rowIndex: number) {
    deleteArticle([{ id: id }]).then((data: Response<string>) => {
        if (data.status === 200) {
            ...
            //删除对应的节点
            instance?.proxy?.$bus.emit("deleteNode", { id: id })
    ...
    deleteArticle(ids).then((data: Response<string>) => {
        if (data.status === 200) {
            ...
            state.page = state.page.filter((item: Article) => {
                //删除对应的节点
                instance?.proxy?.$bus.emit("deleteNode", { id: item.id })
                //如果当前编辑器是当前文章则清空编辑器内容
                instance?.proxy?.$bus.emit("resetArticle", item.id)
                return ids.find((i: ArticleId) => i.id === item.id) == null //过滤已删除的
            })
        } else {
            ElMessage.error(data.message)
        }
